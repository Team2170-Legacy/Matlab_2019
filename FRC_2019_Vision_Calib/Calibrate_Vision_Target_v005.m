...
%         imshow(target_R_img);
        ...
        index = (i - startImg) + 1;
        Calib(index) = Find_Calib_Parameters_v001(target_L_img, target_L_img);
    end
...
%% Take the min/max of Calib parameters from each image
avgCalib = Avg_Calib_Data(Calib)

%% Now write .h file
if target_type==1
    Calibration_Data_Make_h_File_v002(avgCalib, 'FloorLines_Calibration.h');
else % target_type == 2
    Calibration_Data_Make_h_File_v002(avgCalib, 'VisionTarget_Calibration.h');
end
